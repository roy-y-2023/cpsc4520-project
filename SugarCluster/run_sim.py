#!/usr/bin/env python3
"""Run a single Sugarscape simulation with timing and memory measurement."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run a single Sugarscape simulation"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--backend", choices=["slurm", "tamu"], default="tamu",
        help="Execution backend tag written into the timing filename (default: tamu)"
    )
    ...
    if not os.path.exists(manifest):
        ...
        _write_timing_early(timing_path, args.job_id, args.backend, "MANIFEST_MISSING")
        return 1
    ...
    if args.job_id < 1 or args.job_id > total_jobs:
        ...
        print(msg, file=sys.stderr)
        _write_timing_early(timing_path, args.job_id, args.backend, "ID_OUT_OF_RANGE")
        return 1
    ...
    if not os.path.exists(config_path):
        ...
        _write_timing(
            timing_path, args.job_id, args.backend, row, param_names,
...
def _write_timing_early(path: str, job_id: int, backend: str, status: str) -> None:
    """Write a timing stub for early failures."""
    record = {"job_id": job_id, "backend": backend, "status": status, "duration_seconds": 0.0}
    try:
